...
项目使用ESLint和Prettier进行代码质量检查和格式化，确保代码符合规范：
...
4. **代码质量要求**：
   ...
   - 每次开发完成功能后都必须运行ESLint和Prettier检测并格式化
...
**重要**：每次推送代码到远程仓库前，必须执行以下步骤确保代码质量和项目稳定性：

1. **构建验证**：
   - 运行 `npm run build` 确保项目能够成功构建
   - 检查是否生成了 `out` 目录
   - 如果构建失败，必须先修复错误直至成功构建出 `out` 目录
...
5. **代码质量检查**：每次完成功能后都必须运行ESLint和Prettier检测并格式化
   - 运行 `npm run lint` 检查代码质量
   - 运行 `npm run lint:fix` 自动修复可修复的问题
   - 运行 `npm run format` 格式化代码
6. **验证测试**：确保构建成功且功能正常
...
4. 确认响应式设计在各设备上正常
5. 遵循项目现有代码风格
6. **强制要求**：每次完成功能开发后都必须运行ESLint和Prettier检测并格式化
   - 运行 `npm run lint` 检查代码质量
   - 运行 `npm run lint:fix` 自动修复可修复的问题
   - 运行 `npm run format` 格式化代码
   - 确保所有TypeScript文件通过ESLint检查，无严重错误
