...
const SiteSearchResults: React.FC<SiteSearchResultsProps> = ({ query, onClose }) => {
  const { categories } = useNavStore();
  const [filteredResults, setFilteredResults] = useState<{ category: ICategory; websites: IWebsite[] }[]>([]);
  ...
  useEffect(() => {
    ...
    categories.forEach(category => {
      ...
      if (category.title.toLowerCase().includes(query.toLowerCase())) {
        ...
        const matchingWebsites = category.nav.filter(website =>
          ...
          (website.tags && website.tags.some(tag => tag.name && tag.name.toLowerCase().includes(query.toLowerCase())))
        );
          ...
          <div className="p-6 text-center">
            <div className="text-gray-400 dark:text-gray-500 mb-2">
              <svg className="w-12 h-12 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.291-1.1-5.7-2.828-1.409 1.728-3.36 2.828-5.7 2.828a7.962 7.962 0 015.7-5.7c0-.34.034-.674.1-.992H3a1 1 0 00-1 1v9a1 1 0 001 1h18a1 1 0 001-1v-9a1 1 0 00-1-1h-3.1c.066.318.1.652.1.992a7.96 7.96 0 01-5.7 5.7z" />
              </svg>
            </div>
            <p className="text-gray-500 dark:text-gray-400">未找到与 "{query}" 相关的结果</p>
          ...
          <>
            <div className="p-4 border-b border-gray-200/50 dark:border-gray-700/50">
              <h3 className="font-medium text-gray-900 dark:text-white">
                站内搜索结果 ({filteredResults.reduce((acc, curr) => acc + curr.websites.length, 0)})
              </h3>
            </div>
                        ...
                        <div className="flex items-start">
                          {website.icon ? (
                            <img
                              ...
                              onError={(e) => {
                                ...
                                target.parentElement!.innerHTML = `
                                  <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center mr-3 flex-shrink-0">
                                    <span class="text-white text-xs font-bold">${website.name.charAt(0)}</span>
                                  </div>
                                  ${target.parentElement!.innerHTML}
                                `;
                              }}
                            ...
                            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center mr-3 flex-shrink-0">
                              <span className="text-white text-xs font-bold">{website.name.charAt(0)}</span>
                            </div>
                          )}
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-1 mt-2">
                              {website.tags && website.tags.slice(0, 3).map((tag) => (
                                tag.name && (
                                  <span
                                    key={tag.id}
